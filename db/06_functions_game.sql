...
CREATE OR REPLACE FUNCTION process_end_of_cycle()
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    FOR player_rec IN (
        ...
        FOR district_rec IN (
            ...
            SELECT SUM(calculate_ideology_compatibility(player_rec.player_id, p.politician_id))
            INTO ideology_bonus
            FROM politicians p
    ...
    PERFORM create_next_cycle();
END;
$$ LANGUAGE plpgsql;

-- Process specific action
CREATE OR REPLACE FUNCTION process_action(p_action_id UUID)
RETURNS VOID AS $$
DECLARE
    action_rec actions;
    ...
    district_control_rec district_control;
    target_district_control_rec district_control;
    politician_rec politicians;
    relation_rec player_politician_relations;
BEGIN
    -- Get action details
    SELECT * INTO action_rec FROM actions WHERE action_id = p_action_id;
    ...
    CASE action_rec.action_type
            ...
            IF action_rec.target_player_id IS NOT NULL AND control_points > 0 THEN
                ...
                IF target_district_control_rec IS NOT NULL THEN
                    ...
                    INSERT INTO news (
                        ...
                        action_rec.cycle_id,
                        'Attack on Your Control in ' || (SELECT name FROM districts WHERE district_id = action_rec.district_id),
        ...
        WHEN 'reconnaissance' THEN
            IF roll <= success_chance THEN
                ...
                INSERT INTO news (
                    ...
                    action_rec.cycle_id,
                    'Reconnaissance Report: ' || (SELECT name FROM districts WHERE district_id = action_rec.district_id),
            ...
            ELSE
                ...
                INSERT INTO news (
                    ...
                    action_rec.cycle_id,
                    'Partial Reconnaissance: ' || (SELECT name FROM districts WHERE district_id = action_rec.district_id),
        ...
        WHEN 'politician_influence' THEN
            ...
            ELSE
                SELECT * INTO politician_rec FROM politicians WHERE politician_id = action_rec.target_politician_id;
                ...
                IF roll <= success_chance THEN
                    ...
                    ELSE
                        UPDATE player_politician_relations
...
CREATE OR REPLACE FUNCTION process_all_pending_actions(p_cycle_id UUID)
RETURNS INTEGER AS $$
DECLARE
    action_rec RECORD;
...
BEGIN
    ...
    FOR action_rec IN (
        ...
        BEGIN
            PERFORM process_action(action_rec.action_id);
...
CREATE OR REPLACE FUNCTION process_collective_action(p_collective_action_id UUID)
RETURNS VOID AS $$
DECLARE
...
BEGIN
        ...
        WHEN 'attack' THEN
            IF roll <= success_chance THEN
                ...
                IF collective_action_rec.target_player_id IS NOT NULL THEN
                    ...
                    IF target_district_control_rec IS NOT NULL THEN
                        ...
                        INSERT INTO news (
                            ...
                            'Massive Attack on Your Control in ' || (SELECT name FROM districts WHERE district_id = collective_action_rec.district_id),
                ...
                FOR participant_rec IN (
                    SELECT * FROM collective_action_participants
                    WHERE collective_action_id = p_collective_action_id
                ) LOOP
                    ...
                    BEGIN
                        ...
                        INSERT INTO news (
                            ...
                            ' control points in ' || (SELECT name FROM districts WHERE district_id = collective_action_rec.district_id) || '.',
                            'faction',
                            participant_rec.player_id,
                            collective_action_rec.district_id
                        );
            ...
            ELSE
                ...
                FOR participant_rec IN (
                    ...
                    INSERT INTO news (
                        ...
                        'The coordinated attack on ' || (SELECT name FROM districts WHERE district_id = collective_action_rec.district_id) ||
    ...
    INSERT INTO news (
        ...
        collective_action_rec.cycle_id,
        'Major ' || collective_action_rec.action_type || ' Operation in ' || (SELECT name FROM districts WHERE district_id = collective_action_rec.district_id),
...
CREATE OR REPLACE FUNCTION process_all_collective_actions(p_cycle_id UUID)
RETURNS INTEGER AS $$
DECLARE
    action_rec RECORD;
    processed_count INTEGER := 0;
BEGIN
    ...
    FOR action_rec IN (
        ...
        BEGIN
            PERFORM process_collective_action(action_rec.collective_action_id);
            processed_count := processed_count + 1;
        EXCEPTION WHEN OTHERS THEN
            -- Log the error but continue with other actions
...
CREATE OR REPLACE FUNCTION exchange_resource(
    p_player_id UUID,
...
BEGIN
    ...
    SELECT * INTO resources_rec FROM resources WHERE player_id = p_player_id;
...
CREATE OR REPLACE FUNCTION generate_international_effect(
    p_politician_id UUID
)
RETURNS international_effects AS $$
DECLARE
    politician_rec politicians;
    current_cycle cycles;
    effect_type TEXT;
...
BEGIN
    -- Get politician data
    SELECT * INTO politician_rec FROM politicians WHERE politician_id = p_politician_id;
    ...
    IF politician_rec.ideological_leaning <= -3 THEN
        ...
        effect_type := CASE random_int(1, 3)
    ...
    CASE effect_type
        WHEN 'sanctions' THEN
            ...
            resource_effect_type := CASE random_int(1, 4)
        ...
        WHEN 'attack' THEN
            ...
            description := politician_rec.name || ' from ' || politician_rec.country || 
                            ...
                            (SELECT name FROM districts WHERE district_id = target_district_id) || ' district.';
...
CREATE OR REPLACE FUNCTION apply_international_effects()
RETURNS INTEGER AS $$
DECLARE
    effect_rec RECORD;
    player_rec RECORD;
        ...
        FOR player_rec IN (
            ...
            IF effect_rec.control_points_effect IS NOT NULL THEN
                ...
                IF district_control_rec IS NOT NULL THEN
                    ...
                    INSERT INTO news (
                        ...
                        (SELECT name || ' (' || country || ')' FROM politicians WHERE politician_id = effect_rec.politician_id) ||
                        ', your control in ' || 
                        (SELECT name FROM districts WHERE district_id = effect_rec.target_district_id) ||
            ...
            IF effect_rec.resource_effect_type IS NOT NULL AND effect_rec.resource_effect_amount IS NOT NULL THEN
                ...
                INSERT INTO resource_history (
                    ...
                    (SELECT name FROM politicians WHERE politician_id = effect_rec.politician_id)
                );
