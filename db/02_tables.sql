...
COMMENT ON TABLE game.cycles IS 'Game cycles representing morning (12:00 deadline) and evening (18:00 deadline) action periods';
...
COMMENT ON TABLE game.players IS 'Registered players of the game';
...
COMMENT ON TABLE game.districts IS 'Districts of Novi-Sad that can be controlled by players';

-- Politicians (local and international)
CREATE TABLE IF NOT EXISTS game.politicians (
    ...
    district_id UUID REFERENCES game.districts(district_id) ON DELETE SET NULL,
...
COMMENT ON TABLE game.politicians IS 'Politicians who have influence in the game, both local and international';
...
COMMENT ON TABLE game.resources IS 'Resources owned by players (influence, money, information, force)';

-- Resource history
CREATE TABLE IF NOT EXISTS game.resource_history (
    history_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    player_id UUID NOT NULL REFERENCES game.players(player_id) ON DELETE CASCADE,
    cycle_id UUID NOT NULL REFERENCES game.cycles(cycle_id) ON DELETE CASCADE,
...
COMMENT ON TABLE game.resource_history IS 'History of resource changes for audit and tracking purposes';

-- District control
CREATE TABLE IF NOT EXISTS game.district_control (
    control_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    district_id UUID NOT NULL REFERENCES game.districts(district_id) ON DELETE CASCADE,
    player_id UUID REFERENCES game.players(player_id) ON DELETE SET NULL,
    control_points INTEGER NOT NULL DEFAULT 0 CHECK (control_points >= 0),
    last_action_cycle_id UUID REFERENCES game.cycles(cycle_id) ON DELETE SET NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE (district_id, player_id)
);
COMMENT ON TABLE game.district_control IS 'Tracks control points of players in districts';

-- Actions table
CREATE TABLE IF NOT EXISTS game.actions (
    ...
    district_id UUID REFERENCES game.districts(district_id) ON DELETE SET NULL,
    target_player_id UUID REFERENCES game.players(player_id) ON DELETE SET NULL,
    target_politician_id UUID REFERENCES game.politicians(politician_id) ON DELETE SET NULL,
...
COMMENT ON TABLE game.actions IS 'Player actions submitted during game cycles';

-- Collective actions
CREATE TABLE IF NOT EXISTS game.collective_actions (
    collective_action_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    initiator_player_id UUID NOT NULL REFERENCES game.players(player_id) ON DELETE CASCADE,
...
COMMENT ON TABLE game.collective_actions IS 'Collective actions initiated by players for coordinated attacks or defense';

-- Collective action participants
CREATE TABLE IF NOT EXISTS game.collective_action_participants (
    participant_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    collective_action_id UUID NOT NULL REFERENCES game.collective_actions(collective_action_id) ON DELETE CASCADE,
    player_id UUID NOT NULL REFERENCES game.players(player_id) ON DELETE CASCADE, 
...
COMMENT ON TABLE game.collective_action_participants IS 'Participants in collective actions with their resource contributions';

-- Player-politician relations
CREATE TABLE IF NOT EXISTS game.player_politician_relations (
    relation_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    player_id UUID NOT NULL REFERENCES game.players(player_id) ON DELETE CASCADE,
    politician_id UUID NOT NULL REFERENCES game.politicians(politician_id) ON DELETE CASCADE,
...
COMMENT ON TABLE game.player_politician_relations IS 'Friendliness level between players and politicians';

-- News and events
CREATE TABLE IF NOT EXISTS game.news (
    ...
    target_player_id UUID REFERENCES game.players(player_id) ON DELETE SET NULL,
    related_district_id UUID REFERENCES game.districts(district_id) ON DELETE SET NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    created_by UUID REFERENCES game.players(player_id) ON DELETE SET NULL
);
COMMENT ON TABLE game.news IS 'News and events generated during the game';
...
COMMENT ON TABLE game.translations IS 'Translations for game text in English and Russian';

-- International effects
CREATE TABLE IF NOT EXISTS game.international_effects (
    ...
    target_district_id UUID REFERENCES game.districts(district_id) ON DELETE SET NULL,
...
COMMENT ON TABLE game.international_effects IS 'Effects created by international politicians on the game state';
